...
function Profile() {
  ...
  const handleSave = async () => {
    try {
      ...
      if (activeRole === "coach") {
        // Coach mode: update basic info + coach profile via /api/profile/coach
        // The coach endpoint handles first_name/last_name/phone too, so we merge
        const body = {
  ...
  const addRule = async () => {
    ...
    // Create one rule per selected day, sequentially
    // We don't Promise.all because if one fails (e.g. duplicate),
    // we still want the others to succeed.
    const created = [];
    const failed = [];
    ...
    for (const day of activeDays) {
      try {
        const res = await fetch(
          "http://localhost:4000/api/coach/availability",
          {
            ...
            body: JSON.stringify({
              ...
              end_time: endTime, // use the user-selected end_time, not computed
        ...
        const data = await res.json();
        if (!res.ok) {
          failed.push({ day, error: data.error });
          continue;
        }
        created.push({
          id: data.rule.rule_id,
          dayOfWeek: data.rule.day_of_week,
          startTime: data.rule.start_time.slice(0, 5),
          endTime: data.rule.end_time.slice(0, 5),
          duration: data.rule.duration_minutes,
        });
      } catch (err) {
        failed.push({ day, error: err.message });
      }
    }

    // Append created to local state
    if (created.length > 0) {
      setTimeRules((prev) => [...prev, ...created]);
    }

    if (failed.length > 0) {
      alert(
        `Some rules failed:\n${failed
          .map((f) => `${dayMap[f.day]}: ${f.error}`)
          .join("\n")}`
      );
    }
  };
  ...
  // Load existing rules from backend when availability modal opens
  useEffect(() => {
    if (!modalAvail || activeRole !== "coach") return;
    ...
    const loadRules = async () => {
      try {
        ...
        if (!res.ok) return;
        const data = await res.json();
        // Map backend shape → frontend's existing timeRules shape
        setTimeRules(
          (data.rules || []).map((r) => ({
            id: r.rule_id,
            dayOfWeek: r.day_of_week,
            startTime: r.start_time.slice(0, 5), // "09:00:00" → "09:00"
