    ...
    return (
      <div className="aw-page">
        <div className="aw-done-box">
          ...
          <button className="aw-btn-primary" onClick={handleFinishWorkout}>Back to Workouts</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="aw-page">
      <div className="aw-topbar">
        <button className="aw-back-btn" onClick={handleFinishWorkout}>✕ Quit</button>
        <span className="aw-workout-name">{activeWorkout.name}</span>
        <span className="aw-progress">{currentIndex + 1} / {exercises.length}</span>
      </div>

      <div className="aw-progress-bar">
        <div className="aw-progress-fill" style={{ width: `${((currentIndex) / exercises.length) * 100}%` }} />
      </div>

      {phase === "exercise" ? (
        <div className="aw-card">
          <span className="aw-phase-label">Set {currentSet} of {totalSets}</span>
          <h2 className="aw-ex-name">{current.name}</h2>
          <div className="aw-ex-stats">
            <div className="aw-stat"><span className="aw-stat-val">{current.sets}</span><span className="aw-stat-label">Sets</span></div>
            <div className="aw-stat"><span className="aw-stat-val">{current.reps}</span><span className="aw-stat-label">Reps</span></div>
            {current.weight && <div className="aw-stat"><span className="aw-stat-val">{current.weight}lbs</span><span className="aw-stat-label">Weight</span></div>}
          </div>
          <button className="aw-btn-primary" onClick={handleDoneSet}>
            {currentSet < totalSets ? `Done Set ${currentSet} — Rest` : "Done — Next Exercise"}
          </button>
        </div>
      ) : (
        <div className="aw-card">
          ...
          <p className="aw-rest-next">Set {currentSet} of {totalSets} — Up next: {currentSet < totalSets ? `Set ${currentSet + 1}` : (exercises[currentIndex + 1]?.name || "Finish")}</p>
          <button className="aw-btn-secondary" onClick={handleSkipRest}>Skip Rest</button>
        </div>
      )}

      <div className="aw-exercise-queue">
        {exercises.map((ex, i) => (
          <div key={i} className={`aw-queue-item ${i === currentIndex ? "active" : ""} ${i < currentIndex ? "done" : ""}`}>
