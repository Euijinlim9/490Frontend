...
function NutritionistMealPlanPage() {
  ...
  const fetchClients = useCallback(async () => {
    try {
      const res = await fetch("http://localhost:4000/api/nutritionist/clients", {
  ...
  const handleAssign = async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:4000/api/nutritionist/clients/${selectedClient.client_user_id}/meal-plan`,
        { method: "POST", headers, body: JSON.stringify({ name: planName, start_date: startDate, end_date: endDate || null, items }) }
      );
      if (!res.ok) { const d = await res.json(); showToast(d.error || "Failed to assign plan."); return; }
      showToast("Meal plan assigned successfully!");
      setPlanName(""); setStartDate(""); setEndDate(""); setSchedule({});
      ...
      <div className="np-layout">
        ...
        <div className="np-main">
          ...
          <div className="np-tabs">
            <button className={`np-tab ${activeTab === "plans" ? "active" : ""}`} onClick={() => setActiveTab("plans")}>
              Meal Plans
            </button>
            <button className={`np-tab ${activeTab === "meals" ? "active" : ""}`} onClick={() => setActiveTab("meals")}>
              Meal Library
            </button>
          </div>
            ...
            <>
              {!selectedClient ? (
                <div className="np-empty-state">
                  ...
                  <p>{clients.length === 0 ? "You have no clients yet." : "Select a client from the left to assign a meal plan."}</p>
                </div>
              ) : (
                <>
            ...
            <div className="np-meals-section">
              <div className="np-meals-header">
                ...
                <div className="np-meals-controls">
                  <input className="np-input" placeholder="Search meals..." value={mealSearch} onChange={(e) => setMealSearch(e.target.value)} />
                  <button className="np-btn-new" onClick={() => setShowNewMeal(!showNewMeal)}>+ New Meal</button>
                </div>
              </div>
...
export default NutritionistMealPlanPage;
