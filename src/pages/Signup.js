...
function Signup() {
  ...
  const handleCoachSignup = async () => {
    ...
    try {
      ...
      const res = await fetch(buildBackendUrl("/auth/register/coach"), {
        method: "POST",
        body: formData,
      });
      const data = await res.json();
      ...
      <div className="signup-card">
        ...
        {activeTab === "professional" && (
          <div className="signup-form">
            ...
            <button className="signup-btn" onClick={handleProSignup}>
              {getProBtnLabel()}
            </button>
          </div>
        )}
